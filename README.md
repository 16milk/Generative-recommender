...
└── docs/
    ...
    ├── 04-通用生成式召回框架设计.md               # 结合行业实践沉淀的可落地 GR 框架设计
...
最后读 [`docs/04-通用生成式召回框架设计.md`](docs/04-通用生成式召回框架设计.md)，它结合行业生成式召回实践，把本项目需要建设的 GR 框架拆成数据、Tokenizer、模型、训练、Serving、监控六层，并配套一个可直接在浏览器打开的精致 HTML 架构图：[`docs/generative-recommender-architecture.html`](docs/generative-recommender-architecture.html)。
